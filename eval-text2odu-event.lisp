       ...
       (when (eq 
         ...
         (cond
          ((< buffer-tick event-tick)
           ...
           (break "Мы получили сигнал из будущего. Что же теперь делать?")
           )
          ((= buffer-tick event-tick)
           (ПЕРЕРАСКРАСИТЬ-БУФЕР-ЯРО buffer))
          (t
           ;; мы опоздали. Скорее всего будут вперди ещё события по раскраске. Можно ничего не делать.
           (error "Было бы неплохо, если бы мы выявляли ситуации, когда у нас забита очередь требованиями раскраски, но мы пока не умеем это делать"))))))))
  nil)
...
(defun eval-construct-backend-buffer (e)
  "See also CLCO:NOTIFY-ODUVAN-CONSTRUCT-BACKEND-BUFFER"
