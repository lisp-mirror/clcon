Clcon 0.3.9 - Руководство пользователя
...
- F7 в файле системы или 
- (в редакторе) лисп/система/скомпилировать и загрузить систему - для этого система должна быть помечена в первой стоке примерно так: `; -*- coding: utf-8 ; system :clcon-server ; -*-` 
- из консоли `(clco:operate-on-system-for-tcl система 'asdf:load-op . опции)`

Компиляция одной функции лиспа в редакторе: Лисп/Компилировать текущее определение. ПРАВЬМЯ - исходники для отладки запоминаются криво, посмотреть как в SLIME.
...
------------------
Ctrl-Shift-T. 
Отладка потоков кое-как работает, можно пытаться отлаживаться в ГПИ и в консоли. В основном я пытаюсь отлаживаться в ГПИ, но потоки, обозначенные восклицательным знаком, лучше вообще не трогать. См. также [отладка-без-clcon](отладка-без-clcon.md)
