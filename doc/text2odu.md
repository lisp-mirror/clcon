...
и работает только под Linux. Это debug-версия (уже давно не собиралась и, скорее всего, вообще не работает)

Для отладки одуванчика можно сделать, чтобы только первый буфер одуванчика порождал 
соответствующий буфер в лиспе. 
Это облегчает отладку самого редактора, когда выскакивающее окно отладчика и открытие исходного
текста не меняют состояние одуванчика.
Это делается путём разкомментирования строчек, к-рые можно найти по 
.fics buf1
...
Возможные стратегии синхронизации
===================================
Правильная и простая стратегия состоит в том, чтобы всегда блокировать буфер, если нам нужно что-то выполнить со стороны лиспа. Тогда мы можем гарантировать синхронность. Однако такой процесс, как раскраска, происходит долго и мы хотим разбить его в цепочку событий, чтобы редактор не подвисал на каждое нажатие клавиши. Тогда нужно учесть, что наши планы по раскраске, выраженные в запланированных вызовах функций установки тегов теряют смысл, как только что-то поменялось в tcl. 

В этом случае нам поможет "порядковый номер версии текста", tick_count 
