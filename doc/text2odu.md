...
### СДЕЛАНО 1. Научить одуванчик зеркалить происходящее в text.

### ВРОДЕ СДЕЛАНО 2. Система координат для п.1.
...
!2. Зеркалить один буфер из tcl в видимый одуванчик. 

Везде смотрим в лиспе на многопоточность и реентерабельность - не сделано. 
...
!  Норма - когда в состоянии покоя эта опция равна нулю. Если больше нуля - одуванчик захлеб!нулся или что-то сломалось.

3. Автоотступ
Реализовать примитивы:
*do-editing-to-tcl-side* - если истина, то всё редактирование делается не в одуванчике, а отправляется в tcl
синхронизировать insertion point - туда и обратно
включить/выключить режим затупа на стороне tcl
при этом уметь выполнять команды от одуванчика
уметь оповещать одуванчик о выполнении
на стороне одуванчика уметь дождаться выполнения. 
