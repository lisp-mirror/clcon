...
Точки входа в изменение состояния

Freeze - меняет состояние:
0->1 - это значит, его вызвали из обработчика событий. 
1->1 - это ошибка - понадобился вложенный вызов, нужно делать многоуровневую заморозку
2->1 - это значит, его вызвали из обработчика разморозки. Прекратить обработку разморозки.

Unfreeze - меняет состояние
0->0 - это ошибка, но продолжать работу можно
1->2 - и начать процесс разморозки
2->2 - это ошибка, понадобился вложенный вызов, нужно делать многоуровневую заморозку

Скрипт обычного события: если состояние = 0, то выполняться. Иначе - положить в очередь своюветвь, которая не проверяет состояние. Проблема - а что делать с клавиатурными событиями с break? Решение: break должен сработать сразу, до укладывания в очередь.  

Начало процесса разморозки:
  изменить состояние согласно описанному выше для Unfreeze
  продолжение процесса разморозки

Продолжение процесса разморозки
  если состояние процесса разморозки = 2 (оно могло измениться прошлым скриптом-обработчиком на 1)
  получить очередное событие из очереди отложенных
  если нет событий, то поставить состояние процесса разморозки в 0 и выйти. 
  выполнить скрипт одного события, а после себя пусть он вызовет продолжение процесса разморозки. 


  обрамить-скрипт-для-заморозки-разморозки (скрипт,замораживает ли он,break)
  ; эта процедура вызывается для обрабоки каждого скрипта. Параметры:
  ; скрипт - собственно, сами действия
  ; замораживает ли он? если да, то в обрамление вставляем действия по заморозке-разморозке - ПОКА НЕ ДЕЛАЕМ?
  ; add-break - это клавиатурный скрипт, нужно пресечь дальнейшие обработчики на это событие

  Не прокатывает, т.к. есть уже готовые биндинги. Для них контролируем отсутствие слов break , continue, return в скрипте


  Проблема возникнет у нас с любым скриптом, который внутри себя вызывает update, или 
который состоит из нескольких сообщений. Например, Control-O вызывает диалог. Если 
мы не заморозим интерфейс, а Control-O у нас пришло в процессе разморозки, последующие события проскочат до закрытия диалога. 

  Как мы можем защититься? Экспериментально выявлять update (что-то на него запрограммировать, положив в начало очереди, наверное). 

  Но может быть скрипт, который кладёт событие на after <ВРЕМЯ> или after idle. Он не вызывает сам по себе update, но update может возникнуть по другой причине и в другом окне. Например, если мы заморозили, а перед этим был скрипт, положивший что-то на after. Что мы с этим сделаем? Видимо, можем только переименовать after. 


 вызов ф-ии одуванчика. Содержит freeze и вызывает после завершения unfreeze, а ЗАТЕМ continuationbody. Посмотрел, ничего криминального там не вызывается. Однако имете смысл возложить ответственность за вызов unfreeze на user continuation.
        ...
        # possible values:
        # 0 - unfreezed - user input is processed immediately
        # 1 - freezed - user input is put into a query
        # 2 - unfreezing - freezed operation is over, getting events from the queue
        # Entry points to automato state transitions:
        # Freeze - in the event script. 
        # As we receive event from the user that require lisp interaction: 0->1,1->1,2->2 (FreezeA)
        # As we finished processing on lisp side: 0->error,1->2,2->error
        # As we emptied freezed queue, 0->error,1->error,2->0
        # As we get from the freezed queue event that requires lisp interaction:
        #    0->error,1->error,2->1 (FreezeB)
