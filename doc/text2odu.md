...
Скрипт обычного события: если состояние = 0, то выполняться. Иначе - положить в очередь своюветвь, которая не проверяет состояние, а после своего завершения вызывает "Продолжение процесса разморозки". Проблема - а что делать с клавиатурными событиями с break? break должен сработать сразу, до укладывания в очередь.  
